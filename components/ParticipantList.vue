...
<template>
  <div class="w-full max-w-sm rounded-lg bg-gray-100 dark:bg-gray-800 p-4 shadow-lg transition-colors duration-200">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-xl font-bold text-gray-700 dark:text-white transition-colors duration-200">
        Participants
      </h2>
    ...
    >
      ...
      <p class="text-sm text-gray-400 dark:text-gray-500 mt-1 transition-colors duration-200">
    ...
    >
      ...
      >
        ...
        <span class="w-8 text-center text-lg">
          ...
          <template v-else>
            <span
              class="text-blue-600 dark:text-blue-400 font-mono font-bold transition-colors duration-200"
              :aria-label="`Vote: ${participant.vote ?? 'none'}`"
            >{{ participant.vote ?? '-' }}</span>
          </template>
        </span>
      </li>
    </ul>
    ...
    <!-- Room Controls -->
    <div
      v-if="isJoined && roomState.participants.length > 0"
    ...
    >
      ...
      <p
        v-if="status !== 'OPEN'"
        class="text-xs text-center text-gray-500 dark:text-gray-400 mt-2 transition-colors duration-200"
      >
